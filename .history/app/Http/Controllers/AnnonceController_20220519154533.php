...
class AnnonceController extends Controller
{
    ...
    {
        // récupérer les conversations
        $conversations = Conversation::with('userConversation', 'messages')->where([['client_id', Auth::user()->id], ['annonce_id', $id_annonce]],)->first();
        // derniers messages
            $lastMessage = Message::whereRaw('conversation_id=' . $conversations->id . ' and  created_at in (select max(created_at) from messages WHERE conversation_id=' . $conversations->id . ')')->get();
        dd($lastMessages->toArray());
        ...
        return view('annonces.show', compact('annonces', 'vehicules', 'images','conversations','lastMessages'));
    }
    public function reserver($id_annonce, $id_client, $date_debut, $date_fin, $prix)
    {
        $id = $id_annonce;
        $id_c = $id_client;
        $date_debut = $date_debut;
        $date_fin = $date_fin;
        return view('annonces.reserver', compact('id', 'id_c', 'date_debut', 'date_fin', 'prix'));
    }
}
