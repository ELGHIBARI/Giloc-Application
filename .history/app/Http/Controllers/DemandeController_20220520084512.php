...
class DemandeController extends Controller
{
  ...
  public function accept($id_demande, $id_annonce)
  {
    ...
    $client->notify(new AcceptNotification($annonce));
    $contenu="Vous avez accepté la demande du client " . $client->nom_complet ." vous pouvez le contacter via son adresse e-mail : ". $client->email . " ou par son numéro de téléphone : " . $client->tele;
    (new SendEMailController)->index($contenu);
  ...
  public function mesReservation()
  {
    ...
    $demandes = Demande::with('userAnnonce')->where('client_id', Auth::user()->id)->get();
    foreach ($demandes as $demande) {
