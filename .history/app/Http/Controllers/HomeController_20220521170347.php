        ...
        elseif ($role == "partenaire") {
            ...
            foreach ($demandes as $demande) {
                ...
                if ($monEvaluation->etat == 0 && $date_echeance > date('Y-m-d H:i:s')) {
                    ...
                    $evalClient = Evaluation_client::where([['annonce_id', $demande->annonce_id], ['partenaire_id', Auth::user()->id], ['client_id', $client_id]])->update(['etat' => 2]);
                    $evalPartenaire = Evaluation_partenaire::where([['annonce_id', $demande->annonce_id], ['partenaire_id', Auth::user()->id], ['client_id', $client_id]])->update(['etat' => 2]);
                }
            }
