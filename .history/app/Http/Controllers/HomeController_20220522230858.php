    ...
    public function index()
    {
        ...
        elseif ($role == "partenaire") {
            ...
            foreach ($demandes as $demande) {
                ...
                $date_echeance = strtotime($evaluerMonClient->created_at . ' + ' . '7' . ' days');
                if ($evaluerMonClient->etat == 0 && $date_echeance < date('Y-m-d H:i:s')) {
                    ...
                    $monObjet = Evaluation_objet::where([['annonce_id', $demande->annonce_id], ['client_id', $demande->client_id]])->first();
                    if ($monObjet->etat == 0) {
                        $monEvaluation->delete();
                        $monObjet->delete();
                    }
                    if ($monObjet->etat == 1) {
                        $monObjet = Evaluation_objet::where([['annonce_id', $demande->annonce_id],['client_id', $demande->client_id]])->update(['etat' => 2]);
            ...
            foreach ($demandes as $demande) {
                $evaluerMonPartenaire= Evaluation_partenaire::where([['client_id', Auth::user()->id], ['annonce_id', $demande->annonce_id], ['partenaire_id',$demande->userAnnonce->user_id]])->first();
                $evaluerMonPartenaire= Evaluation_partenaire::where([['client_id', Auth::user()->id], ['annonce_id', $demande->annonce_id]])->first();
                $date_echeance = strtotime($evaluerMonPartenaire->created_at . ' + ' . '7' . ' days');
                if ($evaluerMonPartenaire->etat == 0 && $date_echeance > date('Y-m-d H:i:s')) {
                    $evaluerMonPartenaire->delete();
                    $monEvaluationC = Evaluation_client::where([['partenaire_id', $demande->userAnnonce->user_id], ['annonce_id', $demande->annonce_id], ['client_id', Auth::user()->id]])->first();
                    if ($monEvaluationC->etat == 0) {
                        $monEvaluationC->delete();
                    }
                    if ($monEvaluationC->etat == 1) {
                          $monEvaluationC = Evaluation_client::where([['annonce_id', $demande->annonce_id], ['client_id', $demande->client_id],['partenaire_id', $demande->client_id]])->update(['etat' => 2]);
                          $monObjetEval = Evaluation_objet::where([['annonce_id', $demande->annonce_id], ['client_id', $demande->client_id]]);
                          $monEvaluation = Evaluation_partenaire::where([['annonce_id', $demande->annonce_id]], ['partenaire_id', $demande->userAnnonce->user_id], ['client_id',Auth::user()->id]);
                          $monObjetEval->delete();
                          $monEvaluation->delete();
                    }
                } elseif ($evaluerMonPartenaire->etat == 1 && $date_echeance > date('Y-m-d H:i:s')) {
                    //$monEvaluationC = Evaluation_client::where([['partenaire_id', $demande->userAnnonce->user_id], ['annonce_id', $demande->annonce_id], ['client_id', Auth::user()->id]])->first();
                    //$monEvaluationC->delete();
                    $monEvaluationC = Evaluation_client::where([['annonce_id', $demande->annonce_id], ['client_id', $demande->client_id],['partenaire_id', $demande->client_id]]);
                    $monObjetEval = Evaluation_objet::where([['annonce_id', $demande->annonce_id], ['client_id', $demande->client_id]])->update(['etat' => 2]);
                    $monEvaluation = Evaluation_partenaire::where([['annonce_id', $demande->annonce_id]], ['partenaire_id', $demande->userAnnonce->user_id], ['client_id',Auth::user()->id])->update(['etat' => 2]);
                    $monEvaluationC->delete();
                }
            }
