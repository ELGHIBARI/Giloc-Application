...
class HomeController extends Controller
{
    ...
    public function index()
    {
        $role = Auth::user()->role;
        if ($role == "admin") {
            $data = Annonce::select('id', 'created_at')->get()->groupBy(function ($data) {
            ...
            ////////////les demandes par mois
            $datad = Demande::select('id', 'created_at')->get()->groupBy(function ($datad) {
                return Carbon::parse($datad->created_at)->Format('M');
            });

            $monthsd = [];
            $monthCountd = [];
            foreach ($datad as $monthd => $valuesd) {
                $monthsd[] = $monthd;
                $monthCountd[] = Count($valuesd);
            }
            $datau = User::select('id', 'created_at')->orderBy('created_at', 'Desc')->get()->groupBy(function ($datau) {
                return Carbon::parse($datau->created_at)->Format('M');
            });

            $monthsu = [];
            $monthCountu = [];
            foreach ($datau as $monthu => $valuesu) {
                $monthsu[] = $monthu;
                $monthCountu[] = Count($valuesu);
            }
            ...
            $annoncestotale = Annonce::count();
            if($annoncestotale!=0){
                $disponible=((Annonce::where('etat','disponible')->count())/$annoncestotale)*100;
                $attente=((Annonce::where('etat','en_attente')->count())/$annoncestotale)*100;
                $archive=((Annonce::where('etat','archivee')->count())/$annoncestotale)*100;
                $en_location=((Annonce::where('etat','en_location')->count())/$annoncestotale)*100;
                $annonces=[$disponible,$attente,$archive,$en_location];
                }else
                $annonces=[0,0,0,0];

            $datac = [$voitures, $velos, $motos];
            return view('admin.espaceadmin', ['data' => $data, 'months' => $months, 'monthCount' => $monthCount, 'datad' => $datad, 'monthsd' => $monthsd, 'monthCountd' => $monthCountd, 'datac' => $datac, 'monthsu' => $monthsu, 'monthCountu' => $monthCountu, 'datau' => $datau]);
            ...
            foreach ($demandes as $demande) {
                if ($demande->date_fin < date('Y-m-d')) {
                    ...
                    $contenuC = "Cher client votre durée de location est s'écoulée veuillez nous donner votre feedback à propos du partenaire ainsi que l'objet via le lien suivant : " . "http://localhost:8000/evaluations";
                    $contenuP = "Cher partenaire une de vos location a touché sa fin veuillez nous donner votre feedback à propos du client via le lien suivant : " . "http://localhost:8000/evaluations/" . Auth::user()->id;
                    $client = User::where('id', $demande->client_id)->first();
                    (new SendEMailController)->MailEval($contenuC, $contenuP, $client->email, Auth::user()->email);
                }
            }
            $demandes = Demande::with('userAnnonce')->whereHas('userAnnonce', function ($query) {
                $query->where('user_id', '=', Auth::user()->id);
            ...
            foreach ($demandes as $demande) {
                if ($demande->date_fin < date('Y-m-d')) {
                    ...
                    $contenuC = "Cher client votre durée de location est s'écoulée veuillez nous donner votre feedback à propos du partenaire ainsi que l'objet via le lien suivant : " . "http://localhost:8000/evaluations";
                    $contenuP = "Cher partenaire une de vos location a touché sa fin veuillez nous donner votre feedback à propos du client via le lien suivant : " . "http://localhost:8000/evaluations/" . Auth::user()->id;
                    $partenaire = User::where('id',$demande->userAnnonce->user_id)->first();
                    (new SendEMailController)->MailEval($contenuC, $contenuP, Auth::user()->email, $partenaire->email);
                }
            }
            /////////////////////
