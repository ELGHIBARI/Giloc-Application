            ...
            <div class="widget-header  mr-3">
              <ul class="navbar-nav me-auto">
                <li class="nav-item dropdown">
                  <a class="nav-link icon icon-sm rounded-circle border" data-toggle="dropdown" href="#"><i class="fa fa-shopping-cart"></i></a>
                  <span class="badge badge-pill badge-danger notify">{{Auth::user()->unreadNotifications->count()}}</span>
                  <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right px-auto">
                    <span class="dropdown-item dropdown-header">{{Auth::user()->unreadNotifications->count()}} Notifications</span>
                    @foreach(Auth::user()->unreadNotifications as $notification)
                    <div class="dropdown-divider"></div>
                    @php $id=$notification->id;@endphp
                    <strong>votre demande pour l'annonce </strong><br /> {{$notification->data['titre_annonce']}} a été refusé<br /><small><span class="float-right text-muted text-sm"> @php $date_envoi_dem = strtotime($notification->data['date_refus']);
