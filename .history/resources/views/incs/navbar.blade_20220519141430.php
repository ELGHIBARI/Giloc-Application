          ...
          <div class="widgets-wrap float-md-right">
              ...
              <ul class="navbar-nav me-auto">
                <li class="nav-item dropdown">
                  <a class="icon icon-sm rounded-circle border" data-toggle="dropdown" href="#"><i class="fa fa-shopping-cart"></i>
                    <span class="badge badge-pill badge-danger notify">{{Auth::user()->unreadNotifications->where('type','App\Notifications\RefusDemandeNotification')->count()}}</span>
                  </a>
                  <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
                    <span class="dropdown-item dropdown-header">{{Auth::user()->unreadNotifications->where('type','App\Notifications\App\Notifications\RefusDemandeNotification')->count()}} Notifications</span>
                    @foreach(Auth::user()->unreadNotifications->where('type','App\Notifications\App\Notifications\RefusDemandeNotification') as $notification)
                    <div class="dropdown-divider"></div>

                    <a href="{{ route('client.demande') }}" class="dropdown-item text-left" onclick="document.write('<?php $notification->markAsRead(); ?>');">
                      <small><strong>Demande pour l'annonce</strong><br /> {{$notification->data['titre_annonce']}}<br /></small> <small><span class="float-right text-muted text-sm"> @php $date_envoi_dem = strtotime($notification->data['date_refus']);
            ...
            <div class="widget-header  mr-3">
              <ul class="navbar-nav me-auto">
                <li class="nav-item dropdown">
                  <a class="icon icon-sm rounded-circle border" data-toggle="dropdown" href="#"><i class="fa fa-shopping-cart"></i>
                    <span class="badge badge-pill badge-danger notify">{{Auth::user()->unreadNotifications->where('type','App\Notifications\MessageNotification')->count()}}@php @dd(Auth::user()->unreadNotifications->where('type','App\Notifications\MessageNotification')->count())@endphp</span>
                  </a>
                  <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
